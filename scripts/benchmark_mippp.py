        ...
        for solver in solvers:
            ...
            try:
                ...
                for N in values:
                    reps = repetitions(N)
                    runs = [
                        call_executable(["./build/" + exec_name, solver, str(N)])
                ...
                lines = str(e).split("\n")
                error_lines = [
                    line for line in lines if line.lower().find("error") != -1
                ]
                print(
                    f"Skipped {exec_name} {solver}: {error_lines[0] if len(error_lines) > 0 else lines[0]}"
                )
                continue
